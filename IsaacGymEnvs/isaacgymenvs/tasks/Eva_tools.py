...
class Uavs_blue(Uavs_posture):
    ...
    def select_linear_action(self,blue):
        # a=[[0],[0],[0],[0],[0]]
        a=[[0],[0]]

        # print(self.action_space)
        action = self.action_space[0,a]
        # print(action)
        # print(action[:,:,0])
        action[:,:,0] = 0
        action[:,:,1] = 1 
        ...
        return action
    def select_circle_action(self,blue):
        a=[[0],[0]]
        ...
        action = torch.squeeze(action)
        return action
    def select_randome_action(self,blue):
        # a=[[0],[0],[0],[0],[0]]
        a=[[0],[0]]
        ...
        action_3=random.uniform(0,1.7)
        action_2=1/math.cos(action_3)
    ...
    def set_states(self, red_uav, blue_uav, root_states, dis_red_and_blue):
        ...
        pu_pt = torch.squeeze(root_states[:, 0, 0:3] - root_states[:, 2, 0:3])
        pu_pt_oxy = torch.squeeze(root_states[:, 0, 0:2] - root_states[:, 2, 0:2])

        theta_u = get_angle_between_vectors(torch.squeeze(root_states[:, 2, 7:10]), pu_pt)
        theta_t = get_angle_between_vectors(torch.squeeze(root_states[:, 0, 7:10]), pu_pt)

        z_u = torch.squeeze(root_states[:, 2, 2])
