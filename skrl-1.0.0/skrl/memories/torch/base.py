    ...
    def add_samples(self, **tensors: torch.Tensor) -> None:
        ...
        if dim == 2 and shape[0] == self.num_envs:
            for name, tensor in tensors.items():
                if name in self.tensors:
                    ...
                    # print(self.tensors[name][self.memory_index].shape)
            self.memory_index += 1
        # multi environment (number of environments less than num_envs)
            ...
            for name, tensor in tensors.items():
                if name in self.tensors:
                    # print(1)
                    # print(self.tensors[name][self.memory_index])
                    # self.tensors[name][self.memory_index, self.env_index:self.env_index + tensor.shape[0]].copy_(tensor)
                    self.tensors[name][self.memory_index, 0: tensor.shape[0]].copy_(tensor)
        ...
        # comma-separated values
            ...
            with open(memory_path, "a") as file:
                ...
                for i in range(len(self)):
                    ...
                    # writer.writerow(functools.reduce(operator.iconcat, [self.tensors[name][i].tolist() for name in names], []))
        # unsupported format
        else:
    ...
    def load(self, path: str) -> None:
        ...
        if path.endswith(".pt"):
            ...
            for name in self.get_tensor_names():
                ...
                # load_tensors = self.tensors
                self.tensors[name] = getattr(self, f"_tensor_{name}")
                
            # self.memory_index += 199
        ...
        # comma-separated values
            ...
            loaded_data = {name: [] for name in tensor_names}

            with open(path, "r") as file:
                reader = csv.reader(file)
                headers = next(reader)  # Read headers to determine column locations

                # Define the indices for each column based on the CSV format
                action_indices = [i for i, header in enumerate(headers) if header.startswith("actions")]
                next_state_indices = [i for i, header in enumerate(headers) if header.startswith("next_states")]
                state_indices = [i for i, header in enumerate(headers) if header.startswith("states")]
                reward_indices = [i for i, header in enumerate(headers) if header.startswith("rewards")]
                terminated_indices = [i for i, header in enumerate(headers) if header.startswith("terminated")]

                # Read data rows and reconstruct tensors
                for row_idx, row in enumerate(reader):
                    # Ensure the row has the expected number of columns
                    if len(row) < len(headers):
                        print(f"Warning: Row {row_idx} has fewer columns than expected. Skipping row.")
                        continue

                    # Process actions (3 values)
                    actions = [float(row[i]) for i in action_indices]

                    # Process next states (20 values)
                    next_states = [float(row[i]) for i in next_state_indices]

                    # Process states (20 values)
                    states = [float(row[i]) for i in state_indices]

                    # Process rewards (1 value)
                    rewards = float(row[reward_indices[0]])

                    # Process terminated flag (1 value)
                    terminated = 1.0 if row[terminated_indices[0]] == 'True' else 0.0

                    # Add the parsed data to the loaded_data list
                    loaded_data['actions'].append(actions)
                    loaded_data['next_states'].append(next_states)
                    loaded_data['states'].append(states)
                    loaded_data['rewards'].append(rewards)
                    loaded_data['terminated'].append(terminated)

            # Convert lists to tensors and reshape
            for name in loaded_data:
                # Convert to tensor and move to the desired device (CPU or GPU)
                loaded_data[name] = torch.tensor(loaded_data[name], dtype=torch.float32, device=self.device)

                # If necessary, reshape the tensors to match their expected dimensions
                if name in ['actions']:
                    # Actions are of shape (N, 3)
                    loaded_data[name] = loaded_data[name].reshape(-1, 3)
                elif name in ['next_states', 'states']:
                    # States are of shape (N, 20)
                    loaded_data[name] = loaded_data[name].reshape(-1, 20)
                elif name in ['rewards', 'terminated']:
                    # Rewards and terminated are of shape (N,)
                    loaded_data[name] = loaded_data[name].reshape(-1)

                setattr(self, f"_tensor_{name}", loaded_data[name])
            print("Load offline data Successfully!")
            # print(len(self))
        # unsupported format
        else:
