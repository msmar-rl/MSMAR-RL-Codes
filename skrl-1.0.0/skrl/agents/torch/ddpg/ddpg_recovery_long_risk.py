...
from skrl.agents.torch import Agent,SafeAgent,SafeRiskAgent
...
class DDPG_Recovery_Long_Risk(SafeRiskAgent):
    def __init__(self,
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.memory_safe is not None:
            ...
            self._tensors_names = ["states", "second_states","third_states","actions","second_actions","third_actions", "rewards", "terminated","safe_value_1","safe_value_2","safe_value_3"]
    ...
    def record_transition(self,
        ...
        :type timesteps: int
        """
        super().record_transition(states,second_states,third_states, actions,second_actions,third_actions, rewards, terminated, truncated, infos, second_infos,third_infos, timestep, timesteps)
        ...
        if self.memory_safe is not None:
            ...
            self.memory_safe.add_samples(states=states,second_states=second_states,third_states=third_states,
                                        actions=actions,second_actions=second_actions,third_actions=third_actions, rewards=rewards, 
                                        terminated=terminated,safe_value_1=safe_value_1 ,safe_value_2=safe_value_2 ,safe_value_3=safe_value_3, truncated=truncated)
            
            for memory in self.secondary_memories_safe:
                memory.add_samples(states=states,second_states=second_states,third_states=third_states,
                                    actions=actions,second_actions=second_actions,third_actions=third_actions, rewards=rewards, 
                                    terminated=terminated,safe_value_1=safe_value_1 ,safe_value_2=safe_value_2 ,safe_value_3=safe_value_3, truncated=truncated)
                    # if self.memory_unsafe is not None:
    ...
    def _update_safe(self, timestep: int, timesteps: int) -> None:
        ...
        sampled_states, sampled_second_states,sampled_third_states,sampled_actions,sampled_second_actions,sampled_third_actions, sampled_rewards, sampled_dones, sampled_safe_value,sampled_second_safe_value,sampled_third_safe_value = \
