...
class DDPG_Recovery(SafeAgent):
    ...
    def act(self, states: torch.Tensor, safe_value:torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            actions = torch.zeros(100,3).to("cuda:3")
            ...
            actions[positive_mask],_,outputs = self.recovery_policy.act({"states":self._state_preprocessor(states[positive_mask])},role="policy")
            actions[negative_mask],_,outputs = self.reward_policy.act({"states":self._state_preprocessor(states[negative_mask])},role="policy")
