...
class DDPG_Recovery_Risk_Judge(SafeAgent):
    def __init__(self,
        ...
        # self.safe_value_writer = open(f'/home/zwz/Desktop/work1/IsaacGymEnvs/isaacgymenvs/data/ddpg_multi/record_safe_value.txt', 'w', encoding='UTF8')
    ...
    def act(self, states: torch.Tensor, safe_value:torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        if timestep < self._random_timesteps:
            ...
            # self.safe_value_writer.write(f"#{timestep}\n")
            # self.safe_value_writer.write(str(safe_value[0])+"\n")
        ...
        safe_value_step_1 = self.state_risk_critic_1.act({"states":self._state_preprocessor(states,train=False)},role="policy")            
        safe_value_step_2 = self.state_risk_critic_2.act({"states":self._state_preprocessor(states,train=False)},role="policy")            

        # print(safe_value_step_1)
        # print("safe value")
        # print(safe_value)

        safe_value_step_0 = safe_value
        safe_value_step_1 = safe_value_step_1[0].squeeze(-1)
        safe_value_step_2 = safe_value_step_2[0].squeeze(-1)
        
        positive_mask_0 = safe_value>0
        # negative_mask = safe_value<=0

        positive_mask_1 = safe_value_step_1>0
        # negative_mask = safe_value_step_1<=0

        positive_mask_2 = safe_value_step_2>0
        # negative_mask = safe_value_step_2<=0

        positive_mask  = positive_mask_0 | positive_mask_1 | positive_mask_2
        negative_mask  = ~positive_mask
        
        

        safe_count = (positive_mask_0).sum()
        self.track_data("Exploration / safe count (mean)", torch.mean((safe_count).float()).item())

        unsafe_mask_0 = safe_value>0.3     
        safe_count_small = (unsafe_mask_0).sum()
