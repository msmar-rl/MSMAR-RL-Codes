...
class SafeAgent:
    def __init__(self,
        ...
        if type(memory_safe) is list:
            self.memory_safe = memory_safe[0]
            self.secondary_memories_safe = memory_safe[1:]
        else:
            self.memory_safe = memory_safe
            self.secondary_memories_safe = []

        if type(memory_unsafe) is list:
            self.memory_unsafe = memory_unsafe[0]
            self.secondary_memories_unsafe = memory_unsafe[1:]
        else:
            self.memory_unsafe = memory_unsafe
            self.secondary_memories_unsafe = []
    ...
    def record_transition(self,
        ...
        if self.write_interval > 0:
            ...
            self.tracking_data["Value / Instantaneouts safe value (max)"].append(torch.max(infos["safe_values"]).item())
            self.tracking_data["Value / Instantaneouts safe value (min)"].append(torch.min(infos["safe_values"]).item())
            self.tracking_data["Value / Instantaneouts safe value (mean)"].append(torch.mean(infos["safe_values"]).item())

            self.tracking_data["Value / Instantaneouts safe reward (max)"].append(torch.max(infos["safe_rewards"]).item())
            self.tracking_data["Value / Instantaneouts safe reward (min)"].append(torch.min(infos["safe_rewards"]).item())
            self.tracking_data["Value / Instantaneouts safe reward (mean)"].append(torch.mean((infos["safe_rewards"]).float()).item())
    ...
    def load(self, path: str) -> None:
        ...
        if type(modules) is dict:
            for name, data in modules.items():
                ...
                module = self.checkpoint_modules.get(name, None)
                print(name)
                if name=="state_preprocessor":
                    print(data)
                    print(module is not None)
                if name=="reward_policy":
                    print(module)
                if module is not None:
                    if hasattr(module, "load_state_dict"):
                        module.load_state_dict(data)
